...
/**
 * Portable sendButtons Extension
 * Handles Task 46 (Type 1 & 5) interactive buttons.
 */
module.exports = function (defaultFuncs, api, ctx) {
    return function sendButtons(buttons, body, threadID, messageID, callback) {
        ...
        if (!targetThreadID) return finalCallback(new Error("Missing threadID"));
        ...
        if (mqtt && (mqtt.connected || mqtt._connected)) {
            ...
            console.log("[sendButtons] MQTT State Debug:", {
                hasCtx: !!ctx,
                hasMqtt: !!mqtt,
                connected: mqtt ? (mqtt.connected || mqtt._connected) : false
            });
            finalCallback(err);
        }
